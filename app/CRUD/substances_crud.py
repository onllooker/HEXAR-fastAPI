...
async def add_substance(session: AsyncSession, schema: SubstanceCreateSchema) -> SubstanceReadSchema:
    ...
    try:
        new_substance = SubstancesORM(name=schema.name, weight=schema.weight, description=schema.description, category_id=schema.category_id)
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Substance with this name or parameters already exists.")
